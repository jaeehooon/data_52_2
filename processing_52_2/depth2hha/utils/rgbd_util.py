# --*-- coding:utf-8 --*--
import numpy as np
from depth2hha.utils.utils import *
np.seterr(divide='ignore', invalid='ignore')


def process_depth_image(z, missing_mask, C):
    """
    ...
    X, Y, Z = get_point_cloud_from_z(z, C, 1)
    ...
    N1, b1 = compute_normals_square_support(z / 100, missing_mask, normalParam_patchSize[0],
                                            1, C, np.ones(z.shape))
    N2, b2 = compute_normals_square_support(z / 100, missing_mask, normalParam_patchSize[1],
                                            1, C, np.ones(z.shape))
...
def get_point_cloud_from_z(Z, C, s=1):
    """
...
def compute_normals_square_support(depthImage, missingMask, R, sc, cameraMatrix, superpixels):
    """
    ...
    X, Y, Z = get_point_cloud_from_z(depthImage, cameraMatrix, sc)
    Xf = X
    Yf = Y
    Zf = Z
